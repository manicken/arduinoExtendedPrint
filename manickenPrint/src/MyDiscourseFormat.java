 ...
 */
public class MyDiscourseFormat {

  private static Editor editor;
  private static SketchTextArea textarea;
  private static boolean html;

  /**
   * Format and render sketch code.
   */
  public static String GetResult(Editor editor, boolean printInColor, boolean printLineNumbers, String lineNumberSpacing) {
    MyDiscourseFormat.html = printInColor;
    MyDiscourseFormat.editor = editor;
    MyDiscourseFormat.textarea = editor.getCurrentTab().getTextArea();
    ...
    String stopLineStr = Integer.toString(stopLine);
    // Read the code line by line
    for (int i = startLine; i <= stopLine; i++) {
      if (printLineNumbers)
      {
          String linenr = String.format("%1$"+stopLineStr.length()+"s", i);
          cf.append(linenr + lineNumberSpacing);
      }
   ...
   */
  private static void appendToHTML(char c, StringBuilder buffer) {
    if (!html) {
      buffer.append(c);
    } else if (c == '<') {
      buffer.append("&lt;");
  ...
  private static void appendFormattedLine(StringBuilder buffer, int line) {
